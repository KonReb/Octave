...
$headerlines=0
$NewRecordSetText = Get-Content -Path $Filename -Raw 
...
#Write-Host $Alllines[1]

$AllItemNames=$Alllines[$headerlines].Split(";")  #Get the names of items from first line
#Write-Host $AllItemNames

for ($i=$headerlines+1;$i -le $Alllines.Count-1; $i++)
{
